...
**Status:** PRD v3.1 (25 Sep 2026): no custom contracts, positions live in the user's Binance Agentic Wallet. v3: mainnet verification done (go), live Venus parameters, user-chosen risk profiles. v3.1: real Binance Web3 API contract; Agentic Wallet capabilities verified, keeper execution path pending decision (§14).
...
| Pre-market, post-market or overnight session (`marketStatus` ≠ `regular`), or any unknown status | Overnight target |
| Friday 30 min before close, or any MARKET_CLOSED period                  | Overnight target                                                                                                                                                                                                                                                                     |
| Stock on Binance "Upcoming Earnings" list (RWA `/tokens?tabId=3`), or ASSET_PAUSED / ASSET_LIMITED (incl. `earnings`) | Halt target, repay down before the halt starts, do nothing during halt. The API has no earnings date, so list membership is treated as "corporate action now" (conservative). |
...
| **Binance Web3 API: RWA Data**                   | The market brain                        | Base `https://web3.binance.com/build`, HMAC `X-OC-*` auth. `GET /api/v1/dex/market/rwa/price` (`tokenPrice` on-chain vs `referencePrice`), `/tokens` (`platformId=bstock`; `tabId=3` = Upcoming Earnings), `/underlying-market` (`statusInfo.reasonCode`: TRADING, MARKET_CLOSED, MARKET_PAUSED, MARKET_MAINTENANCE, ASSET_PAUSED, ASSET_LIMITED, UNSUPPORTED; `reasonMsg` e.g. cash_dividend, stock_split, earnings; `marketStatus` regular / premarket / postmarket / overnight / closed / pause), `/underlying-profile` (`tokenToShareRatio`, attestation reports as trust signal). No earnings / corporate-action date field. |
...
Nvidia earnings Wednesday after close. Tuesday 22:00 WIB the keeper sees NVDA on the "Upcoming Earnings" list → repays from buffer to bring LTV from 35% to 25% → posts "Protecting: earnings tomorrow, loan reduced by $12, no shares sold." Thursday token resumes, price +6% → keeper restores LTV to 35% → "Credit line raised by $18, available to draw." Dina never opened the app.
...
| RWA API has no earnings / corporate-action date                                                                   | "Upcoming Earnings" list membership → halt target (conservative, may lower the credit line for longer than 24h). Verify the list's window once the API key arrives; switch to a real date if one appears. |
