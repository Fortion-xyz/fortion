...
| Earnings / corporate action within 24h (operator calendar `CORPORATE_ACTIONS`), or ASSET_PAUSED / ASSET_LIMITED (incl. `earnings`) from RWA status | Halt target, repay down before the halt starts, do nothing during halt. The RWA API has no earnings date and its "Upcoming Earnings" filter returns every bStock (verified live 25 Sep), so dates come from an operator-maintained calendar. |
...
| RWA API has no earnings / corporate-action date                                                                   | Operator-maintained calendar (`CORPORATE_ACTIONS`, TICKER=ISO-8601) drives the 24h pre-halt repay; RWA `ASSET_PAUSED` / `ASSET_LIMITED` still catches any halt the calendar misses. Keep the calendar current before the demo. |
